...
export const subscribeToCompetition = (userId: string | number, competitionId: string | number) =>
  http.post(`${base}/api/abonnements/subscribe?userId=${userId}&competitionId=${competitionId}`)
    .then((r: AxiosResponse<any>) => r.data);

export const unsubscribeFromCompetition = (userId: string | number, competitionId: string | number) =>
